...
const analyzeDifferences = (company1Item, company2Item) => {
  const differences = {
    found: false,
    details: {}
  };
  ...
  try {
    ...
    throw new DataProcessingError(
      `Error analyzing differences for transaction ${company1Item.transactionNumber}`,
      error
    );
  }
};
  ...
  try {
    ...
    for (const company1Item of company1Data) {
      ...
      if (!company2Item) {
        ...
        try {
          const differences = analyzeDifferences(company1Item, company2Item);
          ...
          throw new DataProcessingError(
            `Error comparing transaction ${company1Item.transactionNumber}`,
            error
          );
        }
      }
