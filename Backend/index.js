...
const parseDateString = (dateStr, format) => {
  ...
  try {
    ...
    // Create date - note that month needs to be 0-based for JavaScript Date
...
const parseCSV = (filePath, dateFormat) => {
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(filePath)
      .pipe(csv())
      .on('data', (data) => {
        // Log the raw amount before cleaning (for debugging)
...
const analyzeTransactions = (company1Data, company2Data) => {
  ...
  // Calculate totals for open transactions
  ...
  // Analyze each transaction from company1
  ...
  // Add remaining company2 items to unmatched
